...
import org.xinghe.AIThermalGuardIoT.common.config.StorageConfigProperties;
...
public class FileStorageService{
    ...
    public byte[] downloadFile(String fileKey) {
        if(!fileExists(fileKey)){
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "file is not existing: " + fileKey);
        }
        ...
        try {
            ...
            log.error("Downloading failed:{} - {}", fileKey, e.getMessage(), e);
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "Downloading failed" + e.getMessage());
        }
    }
    ...
    private String uploadFile(MultipartFile file, String prefix) {
        ...
        try (InputStream stream = file.getInputStream()){
            ...
            s3Client.putObject(putRequest, RequestBody.fromInputStream(stream, file.getSize()));
            log.info("Uploading success: {} - {}", originalFilename, fileKey);
            return fileKey;
        } catch (IOException e) {
            log.error("Reading file failed: {}", e.getMessage(),e);
            throw new BusinessException(ErrorCode.STORAGE_UPLOAD_FAILED,"Reading failed");
        } catch (S3Exception e) {
            log.error("Uploading file failed: {}", e.getMessage(),e);
            throw new BusinessException(ErrorCode.STORAGE_UPLOAD_FAILED,"Uploading failed: " + e.getMessage());
        }
    ...
    public boolean fileExists(String fileKey) {
        try {
            ...
            log.warn("Existing Checking failed:{} - {}", fileKey, e.getMessage());
            return false;
        }
    ...
    public long getFileSize(String fileKey) {
        try {
            ...
            log.warn("Getting length failed:{} - {}", fileKey, e.getMessage());
            throw new BusinessException(ErrorCode.STORAGE_DOWNLOAD_FAILED, "Getting length failed");
        }
    }
    ...
    private void deleteFile(String fileKey) {
        if (fileKey == null || fileKey.isEmpty()) {
            log.debug("skip deleting for empty file ");
            return;
        }
